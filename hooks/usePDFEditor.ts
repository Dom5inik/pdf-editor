...
export function usePDFEditor() {
    ...
    const processPDF = useCallback(async (file: File, sourceId: string) => {
        ...
        // Use a copy of the buffer for PDF.js to prevent the original buffer from being detached
        ...
        for (let i = 1; i <= numPages; i++) {
            pagesPromises.push(
                (async () => {
                    ...
                    // Generate thumbnail for the page list sidebar.
                    // We scale the page to a fixed width (180px) and render it to a canvas.
    ...
    const loadPDF = useCallback(async (file: File) => {
        ...
        try {
            ...
            setState(prev => ({
                ...prev,
                pdfDocuments: { [sourceId]: file }, // Store File instead of buffer to prevent detachment issues
                pages: newPages,
                fileName: file.name,
                isLoading: false,
                isAppending: false,
    ...
    const appendPDF = useCallback(async (file: File) => {
        ...
        try {
            ...
            setState(prev => ({
                ...prev,
                pdfDocuments: { ...prev.pdfDocuments, [sourceId]: file },
            ...
            alert('Fehler beim Hinzufügen der PDF-Datei');
        }
    }, [processPDF]);
                ...
                p.id === pageId
                    ? {
                        ...
                        elements: p.elements.map(el => {
                            if (el.id === elementId) {
                                // Deep merge 'content' property if it's being updated.
                                // This prevents losing text properties when only updating position (x,y).
